...
{
    /// <summary>
    /// Android Renderer of <see cref="Iratrips.MapKit.MKCustomMap"/>
    /// </summary>
...

    {
        ...
        internal MKCustomMap FormsMap
        {
            get { return Element as MKCustomMap; }
        }
        ...
        /// <summary>
        /// Creates a new instance of <see cref="MKCustomMapRenderer"/>
        /// </summary>
        /// <param name="context">Android context</param>
        public MKCustomMapRenderer(Context context) : base(context)
        { }
        ...
        void FormsMapPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                case nameof(MKCustomMap.Pins):
                    UpdatePins();
                    break;
                case nameof(MKCustomMap.SelectedPin):
                    SetSelectedItem();
                    break;
                case nameof(MKCustomMap.Polylines):
                    UpdateLines();
                    break;
                case nameof(MKCustomMap.Circles):
                    UpdateCircles();
                    break;
                case nameof(MKCustomMap.Polygons):
                    UpdatePolygons();
                    break;
                case nameof(MKCustomMap.Routes):
                    UpdateRoutes();
                    break;
                case nameof(MKCustomMap.TilesUrlOptions):
                    UpdateTileOptions();
                    break;
                case nameof(MKCustomMap.ShowTraffic):
                    UpdateShowTraffic();
                    break;
                case nameof(MKCustomMap.MapRegion):
                    UpdateMapRegion();
                    break;
                case nameof(MKCustomMap.IsClusteringEnabled):
                    UpdateIsClusteringEnabled();
                    break;
                case nameof(MKCustomMap.MapType):
                    UpdateMapType();
                    break;
                case nameof(MKCustomMap.IsShowingUser):
                    UpdateIsShowingUser();
                    break;
                case nameof(MKCustomMap.HasScrollEnabled):
                    UpdateHasScrollEnabled();
                    break;
                case nameof(MKCustomMap.HasZoomEnabled):
        ...
        void OnCustomPinsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                foreach (MKCustomMapPin pin in e.NewItems)
                {
                    AddPin(pin);
                }
            }
            ...
            {
                foreach (MKCustomMapPin pin in e.OldItems)
                {
        ...
        void OnLineCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (MKPolyline line in e.NewItems)
                {
                    AddLine(line);
                }
            }
            ...
            {
                foreach (MKPolyline line in e.OldItems)
                {
        ...
        {
            var line = (MKPolyline)sender;

            if (e.PropertyName == nameof(MKPolyline.LineCoordinates))
            {
            ...
            else if (e.PropertyName == nameof(MKPolyline.Color))
            {
                _polylines[line].Color = line.Color.ToAndroid().ToArgb();
            }
            else if (e.PropertyName == nameof(MKPolyline.LineWidth))
            {
        ...
        void RemovePin(MKCustomMapPin pin, bool removeMarker = true)
        {
        ...
        void SetSelectedItem()
        {
            ...
            if (FormsMap.SelectedPin != null)
            {
                ...
                if (FormsMap.SelectedPin.Callout != null)
                {
        ...
        void OnRouteCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (MKRoute route in e.NewItems)
                {
                    AddRoute(route);
                }
            }
            ...
            {
                foreach (MKRoute route in e.OldItems)
                {
        ...
        {
            var route = (MKRoute)sender;

            if (e.PropertyName == nameof(MKRoute.Source) ||
                e.PropertyName == nameof(MKRoute.Destination) ||
                e.PropertyName == nameof(MKRoute.TravelMode))
            {
                route.PropertyChanged -= OnRoutePropertyChanged;
                _routes[route].Remove();
                _routes.Remove(route);
        ...
        void OnPolygonsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (MKPolygon poly in e.NewItems)
                {
                    AddPolygon(poly);
                }
            }
            ...
            {
                foreach (MKPolygon poly in e.OldItems)
                {
        ...
        void CirclesCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (MKCircle circle in e.NewItems)
                {
                    AddCircle(circle);
                }
            }
            ...
            {
                foreach (MKCircle circle in e.OldItems)
                {
        ...
        void AddCircle(MKCircle circle)
        {
            circle.PropertyChanged += CirclePropertyChanged;
        ...
        async void AddRoute(MKRoute route)
        {
        ...
        void SetRouteData(MKRoute route, GmsRouteResult routeResult)
        {
            ...
            var steps = new MKRouteStep[apiSteps.Count()];
            var routeFunctions = (IRouteFunctions)route;


            for (int i = 0; i < steps.Length; i++)
            {
                steps[i] = new MKRouteStep();
            ...
            if (FormsMap.TilesUrlOptions != null)
            {
                ...
                _tileOverlay = _googleMap.AddTileOverlay(
                    new TileOverlayOptions()
                        .InvokeTileProvider(
                            new MKCustomTileProvider(FormsMap.TilesUrlOptions))
                        .InvokeZIndex(-1));
            }
        }
        /// <summary>
        ...
        /// </summary>
        void UnregisterCollections(MKCustomMap map)
        {
        ...
        /// <summary>
        /// Gets the <see cref="MKCustomMapPin"/> by the native <see cref="Marker"/>
        /// </summary>
        /// <param name="marker">The marker to search the pin for</param>
        /// <returns>The forms pin</returns>
        protected MKCustomMapPin GetPinByMarker(Marker marker)
        {

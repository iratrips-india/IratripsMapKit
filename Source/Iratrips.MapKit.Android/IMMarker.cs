...
namespace Iratrips.MapKit.Droid
{
    ...
    /// </summary>
    internal class IMMarker : Java.Lang.Object, IClusterItem
    {
        Context _context;
        /// <summary>
        /// Creates a new instance of <see cref="IMMarker"/>
        /// </summary>
        /// <param name="pin">The intnernal pin</param>
        /// <param name="context">Android context</param>
        public IMMarker(IMCustomMapPin pin, Context context)
        {
            Pin = pin;
            _context = context;
        }
        ...
        /// </summary>
        public IMCustomMapPin Pin { get;  set; }
        /// <summary>
        ...
        public async Task HandlePropertyChangedAsync(PropertyChangedEventArgs e, bool isDragging)
        {
            ...
            {
                case nameof(IMCustomMapPin.Title):
                    Marker.Title = Pin.Title;
                    break;
                case nameof(IMCustomMapPin.Subtitle):
                    Marker.Snippet = Pin.Subtitle;
                    break;
                case nameof(IMCustomMapPin.Image):
                    await UpdateImageAsync();
                    break;
                case nameof(IMCustomMapPin.DefaultPinColor):
                    await UpdateImageAsync();
                    break;
                case nameof(IMCustomMapPin.Position):
                    if (!isDragging)
                    {
                        Marker.Position = new LatLng(Pin.Position.Latitude, Pin.Position.Longitude);
                    }
                    break;
                case nameof(IMCustomMapPin.IsVisible):
                    Marker.Visible = Pin.IsVisible;
                    break;
                case nameof(IMCustomMapPin.Anchor):
                    if (Pin.Image != null)
                    {
                        Marker.SetAnchor((float)Pin.Anchor.X, (float)Pin.Anchor.Y);
                    }
                    break;
                case nameof(IMCustomMapPin.IsDraggable):
                    Marker.Draggable = Pin.IsDraggable;
                    break;
                case nameof(IMCustomMapPin.Rotation):

...
[assembly: ExportRenderer(typeof(IMCustomMap), typeof(IMCustomMapRenderer))]
namespace Iratrips.MapKit.Droid
{
    /// <summary>
    /// Android Renderer of <see cref="TK.CustomMap.TKCustomMap"/>
    /// </summary>
    public class IMCustomMapRenderer : ViewRenderer<IMCustomMap, MapView>, IRendererFunctions, GoogleMap.ISnapshotReadyCallback, GoogleMap.IOnCameraIdleListener, IOnMapReadyCallback
    {
        ...
        readonly List<IMRoute> _tempRouteList = new List<IMRoute>();

        readonly Dictionary<IMRoute, Polyline> _routes = new Dictionary<IMRoute, Polyline>();
        readonly Dictionary<IMPolyline, Polyline> _polylines = new Dictionary<IMPolyline, Polyline>();
        readonly Dictionary<IMCircle, Circle> _circles = new Dictionary<IMCircle, Circle>();
        readonly Dictionary<IMPolygon, Polygon> _polygons = new Dictionary<IMPolygon, Polygon>();
        readonly Dictionary<IMCustomMapPin, IMMarker> _markers = new Dictionary<IMCustomMapPin, IMMarker>();
        ...
        internal IMCustomMap FormsMap
        {
            get { return Element as IMCustomMap; }
        }
        ...
        /// <summary>
        /// Creates a new instance of <see cref="IMCustomMapRenderer"/>
        /// </summary>
        /// <param name="context">Android context</param>
        public IMCustomMapRenderer(Context context) : base(context)
        { }

        /// <inheritdoc />
        protected override void OnElementChanged(ElementChangedEventArgs<IMCustomMap> e)
        {
            if (!IMGoogleMaps.IsInitialized) throw new Exception("Call TKGoogleMaps.Init first");
            ...
            lock (_lockObj)
            {
                ...
                if (e.OldElement != null)
                {
                    ...
                    UnregisterCollections((IMCustomMap)e.OldElement);
        ...
        void FormsMapPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                case nameof(IMCustomMap.Pins):
                    UpdatePins();
                    break;
                case nameof(IMCustomMap.SelectedPin):
                    SetSelectedItem();
                    break;
                case nameof(IMCustomMap.Polylines):
                    UpdateLines();
                    break;
                case nameof(IMCustomMap.Circles):
                    UpdateCircles();
                    break;
                case nameof(IMCustomMap.Polygons):
                    UpdatePolygons();
                    break;
                case nameof(IMCustomMap.Routes):
                    UpdateRoutes();
                    break;
                case nameof(IMCustomMap.TilesUrlOptions):
                    UpdateTileOptions();
                    break;
                case nameof(IMCustomMap.ShowTraffic):
                    UpdateShowTraffic();
                    break;
                case nameof(IMCustomMap.MapRegion):
                    UpdateMapRegion();
                    break;
                case nameof(IMCustomMap.IsClusteringEnabled):
                    UpdateIsClusteringEnabled();
                    break;
                case nameof(IMCustomMap.MapType):
                    UpdateMapType();
                    break;
                case nameof(IMCustomMap.IsShowingUser):
                    UpdateIsShowingUser();
                    break;
                case nameof(IMCustomMap.HasScrollEnabled):
                    UpdateHasScrollEnabled();
                    break;
                case nameof(IMCustomMap.HasZoomEnabled):
        ...
        public virtual void OnMapReady(GoogleMap googleMap)
        {
            lock (_lockObj)
            {
                ...
                if (FormsMap.IsClusteringEnabled)
                {
                    ...
                    _clusterManager.Renderer = new IMMarkerRenderer(Context, _googleMap, _clusterManager, this);
                }
        ...
        void OnCustomPinsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                foreach (IMCustomMapPin pin in e.NewItems)
                {
                    AddPin(pin);
                }
            }
            ...
            {
                foreach (IMCustomMapPin pin in e.OldItems)
                {
        ...
        {
            var pin = sender as IMCustomMapPin;
            if (pin == null) return;


            IMMarker marker = null;
            if (!_markers.ContainsKey(pin) || (marker = _markers[pin]) == null) return;
            await marker.HandlePropertyChangedAsync(e, _isDragging);

            if (FormsMap.IsClusteringEnabled && e.PropertyName == nameof(IMCustomMapPin.Position) && !_isDragging)
            {
        ...
        void OnLineCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (IMPolyline line in e.NewItems)
                {
                    AddLine(line);
                }
            }
            ...
            {
                foreach (IMPolyline line in e.OldItems)
                {
        ...
        {
            var line = (IMPolyline)sender;

            if (e.PropertyName == nameof(IMPolyline.LineCoordinates))
            {
            ...
            else if (e.PropertyName == nameof(IMPolyline.Color))
            {
                _polylines[line].Color = line.Color.ToAndroid().ToArgb();
            }
            else if (e.PropertyName == nameof(IMPolyline.LineWidth))
            {
        ...
        void RemovePin(IMCustomMapPin pin, bool removeMarker = true)
        {
        ...
        void OnRouteCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (IMRoute route in e.NewItems)
                {
                    AddRoute(route);
                }
            }
            ...
            {
                foreach (IMRoute route in e.OldItems)
                {
        ...
        {
            var route = (IMRoute)sender;

            if (e.PropertyName == nameof(IMRoute.Source) ||
                e.PropertyName == nameof(IMRoute.Destination) ||
                e.PropertyName == nameof(IMRoute.TravelMode))
            {
                route.PropertyChanged -= OnRoutePropertyChanged;
                _routes[route].Remove();
                _routes.Remove(route);
        ...
        void OnPolygonsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (IMPolygon poly in e.NewItems)
                {
                    AddPolygon(poly);
                }
            }
            ...
            {
                foreach (IMPolygon poly in e.OldItems)
                {
        ...
        void AddPolygon(IMPolygon polygon)
        {
        ...
        void CirclesCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                foreach (IMCircle circle in e.NewItems)
                {
                    AddCircle(circle);
                }
            }
            ...
            {
                foreach (IMCircle circle in e.OldItems)
                {
        ...
        void AddCircle(IMCircle circle)
        {
            circle.PropertyChanged += CirclePropertyChanged;
        ...
        async void AddRoute(IMRoute route)
        {
        ...
        void SetRouteData(IMRoute route, GmsRouteResult routeResult)
        {
            ...
            var steps = new IMRouteStep[apiSteps.Count()];
            var routeFunctions = (IRouteFunctions)route;


            for (int i = 0; i < steps.Length; i++)
            {
                steps[i] = new IMRouteStep();
        ...
        async Task UpdateImage(IMCustomMapPin pin, MarkerOptions markerOptions)
        {
        ...
        async Task UpdateImage(IMCustomMapPin pin, Marker marker)
        {
            BitmapDescriptor bitmap;
            try
            {
            ...
            if (FormsMap.TilesUrlOptions != null)
            {
                ...
                _tileOverlay = _googleMap.AddTileOverlay(
                    new TileOverlayOptions()
                        .InvokeTileProvider(
                            new IMCustomTileProvider(FormsMap.TilesUrlOptions))
                        .InvokeZIndex(-1));
            }
        }
        /// <summary>
        ...
        /// </summary>
        void UnregisterCollections(IMCustomMap map)
        {
        ...
        /// <summary>
        /// Gets the <see cref="IMCustomMapPin"/> by the native <see cref="Marker"/>
        /// </summary>
        /// <param name="marker">The marker to search the pin for</param>
        /// <returns>The forms pin</returns>
        protected IMCustomMapPin GetPinByMarker(Marker marker)
        {
